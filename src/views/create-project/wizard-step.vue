<template>
    <li :class="{active:tab.active, checked: tab.checked, 'max-step': index === maxStep, square_shape:isStepSquare, tab_shape:isTabShape}">
                ...
                <transition :name="transition" mode="out-in">
                    ...
                    >
                        <slot name="active-step">
                            <i v-if="tab.icon" :class="tab.icon" class="wizard-icon" />
                            <i v-else class="wizard-icon">{{ index + 1 }}</i>
                        </slot>
                    </div>
                    <slot v-if="!tab.active">
                        <i v-if="!tab.active && tab.icon" :class="tab.icon" class="wizard-icon" />
